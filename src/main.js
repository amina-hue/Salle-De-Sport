...
// const fs   = require('fs');
// const os   = require('os');
// const { initDatabase, getConfig, getDb } = require('./db');
...
// // ══════════════════════════════════════════════
// const createMysqlConfigWindow = (errorMsg = null) => {
//   const html = `
//     <!DOCTYPE html>
//     <html>
//     <head>
//       <meta charset="UTF-8">
//       <title>FitManager — Configuration MySQL</title>
//       <style>
...
//         <p class="subtitle">Entre les informations de connexion à ta base de données MySQL.</p>
...
//         <input type="text" id="user" value="root" placeholder="root" />

//         <label>Mot de passe</label>
//         <input type="password" id="password" placeholder="Ton mot de passe MySQL" />

//         <p class="hint">💡 Le mot de passe que tu as défini lors de l'installation de MySQL</p>

//         <button class="btn" id="connectBtn" onclick="connect()">Se connecter</button>
//       </div>
...
//             user: document.getElementById('user').value.trim() || 'root',
//             password: document.getElementById('password').value,
//           };

//           ipcRenderer.send('try-mysql-config', config);
//         }
...
//   const tmpPath = path.join(os.tmpdir(), 'fitmanager-mysql-config.html');
//   fs.writeFileSync(tmpPath, html);

//   const configWindow = new BrowserWindow({
//     width: 560, height: 580, resizable: false,
//     title: 'FitManager — Configuration MySQL',
//     webPreferences: { nodeIntegration: true, contextIsolation: false },
//   });

//   configWindow.setMenuBarVisibility(false);
//   configWindow.loadFile(tmpPath);
//   return configWindow;
// };
// const createAdminWindow = (onDone) => {
//   const html = `
//     <!DOCTYPE html>
//     <html>
//     <head>
//       <meta charset="UTF-8">
//       <title>FitManager — Créer votre compte</title>
//       <style>
//         * { box-sizing: border-box; margin: 0; padding: 0; }
//         body {
//           font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
...
//         }
//         input:focus { border-color: #3b82f6; }
//         .error { background: #450a0a; border: 1px solid #991b1b; border-radius: 8px; padding: 12px 16px; margin-top: 16px; font-size: 13px; color: #fca5a5; }
//         .btn { margin-top: 24px; width: 100%; padding: 12px; background: #e53935; color: white; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; }
...
//       <div class="card">
//         <div class="icon">👤</div>
//         <h1>Créer votre compte administrateur</h1>
//         <p class="subtitle">Ce compte vous permettra de gérer votre salle de sport.</p>
//         <div id="error"></div>
//         <label>Nom</label>
...
//             document.getElementById('error').innerHTML = '<div class="error">❌ Veuillez remplir tous les champs</div>';
//             return;
//           }
//           ipcRenderer.send('create-admin', { nom, prenom, email, motDePasse: password });
//         }
//       </script>
//     </body>
//     </html>
...
//   const tmpPath = path.join(os.tmpdir(), 'fitmanager-admin.html');
//   fs.writeFileSync(tmpPath, html);

//   const adminWindow = new BrowserWindow({
//     width: 560, height: 620, resizable: false,
//     title: 'FitManager — Créer votre compte',
//     webPreferences: { nodeIntegration: true, contextIsolation: false },
//   });

//   adminWindow.setMenuBarVisibility(false);
//   adminWindow.loadFile(tmpPath);

//   ipcMain.once('create-admin', async (event, data) => {
//     const { nom, prenom, email, motDePasse } = data;
//     await query(
//       'INSERT INTO Utilisateur (nom, prenom, email, motDePasse, role_id) VALUES (?, ?, ?, ?, 1)',
//       [nom, prenom, email, motDePasse]
//     );
//     adminWindow.close();
//     sendInstallNotification({ nom, prenom, email });
//     onDone();
//   });
// };
// const sendInstallNotification = async (adminData) => {
//   const transporter = createTransporter();
//   try {
//     await transporter.sendMail({
//       from: '"FitManager" <tinhinanethequeen@gmail.com>',
//       to: 'tinhinanethequeen@gmail.com',
//       subject: '🏋️ Nouvelle installation FitManager',
//       html: `
//         <div style="font-family:sans-serif;padding:24px;background:#f8fafc">
//           <h2 style="color:#e53935">🏋️ Nouvelle installation FitManager</h2>
//           <p>Un nouvel administrateur vient de créer son compte :</p>
//           <table style="border-collapse:collapse;margin-top:16px">
//             <tr><td style="padding:8px;font-weight:bold">Nom</td><td style="padding:8px">${adminData.nom} ${adminData.prenom}</td></tr>
//             <tr><td style="padding:8px;font-weight:bold">Email</td><td style="padding:8px">${adminData.email}</td></tr>
//             <tr><td style="padding:8px;font-weight:bold">Date</td><td style="padding:8px">${new Date().toLocaleString('fr-FR')}</td></tr>
//             <tr><td style="padding:8px;font-weight:bold">OS</td><td style="padding:8px">${process.platform === 'win32' ? 'Windows' : process.platform === 'darwin' ? 'macOS' : 'Linux'}</td></tr>
//           </table>
//         </div>
//       `,
//     });
...
//   // Essaie de se connecter avec la config sauvegardée
//   const savedConfig = getConfig();

//   if (savedConfig) {
//     try {
//       await initDatabase(savedConfig);
//       createWindow();
//       return;
//     } catch (err) {
//       console.error('Config sauvegardée invalide:', err.message);
//       // Config invalide → afficher l'écran de config avec erreur
//     }
//   }

//   // Pas de config ou config invalide → afficher l'écran de config
//   let configWin = createMysqlConfigWindow();

//   ipcMain.on('try-mysql-config', async (event, config) => {
//     try {
...
//     } catch (err) {
//       configWin.close();
//       configWin = createMysqlConfigWindow(
//         err.message.includes('Access denied')
//           ? 'Mot de passe incorrect'
//           : err.message.includes('ECONNREFUSED')
//           ? 'MySQL non démarré ou hôte incorrect'
//           : err.message
//       );
//     }
//   });
...
const createMysqlConfigWindow = (errorMsg = null) => {
  const html = `
    ...
    <head>
      ...
      <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #0f172a; color: #e2e8f0; display: flex; align-items: center; justify-content: center; min-height: 100vh; padding: 32px; }
        .card { background: #1e293b; border: 1px solid #334155; border-radius: 16px; padding: 40px; max-width: 480px; width: 100%; box-shadow: 0 25px 50px rgba(0,0,0,0.5); }
        .icon { font-size: 48px; margin-bottom: 16px; }
        h1 { font-size: 22px; color: #f8fafc; margin-bottom: 8px; }
        .subtitle { color: #94a3b8; font-size: 14px; margin-bottom: 28px; }
        label { display: block; font-size: 13px; font-weight: 600; color: #94a3b8; margin-bottom: 6px; margin-top: 16px; }
        ...
        input:focus { border-color: #3b82f6; }
        .error { background: #450a0a; border: 1px solid #991b1b; border-radius: 8px; padding: 12px 16px; margin-top: 16px; font-size: 13px; color: #fca5a5; }
        .btn { margin-top: 24px; width: 100%; padding: 12px; background: #3b82f6; color: white; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; }
        .btn:hover { background: #2563eb; }
        .btn:disabled { background: #1e3a5f; cursor: not-allowed; }
        .hint { font-size: 12px; color: #475569; margin-top: 8px; }
      </style>
...
const createAdminWindow = (onDone) => {
  ...
  const adminWindow = new BrowserWindow({
    width: 560, height: 600, resizable: false,
    title: 'FitManager — Créer votre compte',
    webPreferences: { nodeIntegration: true, contextIsolation: false },
  });
  adminWindow.setMenuBarVisibility(false);
...
const sendInstallNotification = async (adminData) => {
  ...
  try {
    await transporter.sendMail({
      from: '"FitManager" <tinhinanethequeen@gmail.com>',
      to: 'amina.albane@se.univ-bejaia.dz',
      subject: '🏋️ Nouvelle installation FitManager',
      html: `
        <div style="font-family:sans-serif;padding:24px;background:#f8fafc">
          <h2 style="color:#e53935">🏋️ Nouvelle installation FitManager</h2>
    ...
    console.log('✅ Notification envoyée à amina.albane@se.univ-bejaia.com');
  } catch (err) {
...
app.whenReady().then(async () => {
  ...
  const startApp = async () => {
    ...
    const users = await query('SELECT COUNT(*) as count FROM Utilisateur');
    if (users[0].count === 0) {
      await new Promise((resolve) => createAdminWindow(resolve));
    }
    createWindow();
  };
  ...
  ipcMain.on('try-mysql-config', async (event, config) => {
    try {
      ...
      configWin.close();
      configWin = createMysqlConfigWindow(
        err.message.includes('Access denied') ? 'Mot de passe incorrect' :
        err.message.includes('ECONNREFUSED') ? 'MySQL non démarré ou hôte incorrect' :
        err.message
      );
    }
  });
...
ipcMain.handle('getEmailsAdherentsActifs', async () => {
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT ad.email, ad.prenom, ad.nom FROM Adherent ad
       JOIN Abonnement ab ON ab.adherent_id = ad.idAdherent
       WHERE ab.statut = 'actif' AND ad.email IS NOT NULL AND ad.email != ''`,
      (err, result) => { if (err) reject(err); else resolve(result); }
    );
...
ipcMain.handle('login', async (event, data) => {
  ...
  if (!results.length) return { success: false, message: 'Email ou mot de passe incorrect' };
  const ok = await bcrypt.compare(motDePasse, results[0].motDePasse);
  if (!ok) return { success: false, message: 'Email ou mot de passe incorrect' };
...
ipcMain.handle('updateAbonnement', async (event, data) => {
  return new Promise((resolve, reject) => {
    const { idAbonnement, type_id, dateDebut, dateFin, statut, montantDu, dureeSuspension, causeSuspension, dateFinSuspension } = data;
    const isSuspension = statut === 'suspendu' && dureeSuspension;
    const sql = isSuspension
      ? `UPDATE Abonnement SET type_id=?, dateDebut=?, dateFin=DATE_ADD(?, INTERVAL ? DAY), statut=?, montantDu=?, dureeSuspension=?, causeSuspension=?, dateFinSuspension=? WHERE idAbonnement=?`
      : `UPDATE Abonnement SET type_id=?, dateDebut=?, dateFin=?, statut=?, montantDu=?, dureeSuspension=?, causeSuspension=?, dateFinSuspension=? WHERE idAbonnement=?`;
    const params = isSuspension
      ? [type_id, dateDebut, dateFin, dureeSuspension, statut, montantDu??null, dureeSuspension, causeSuspension??null, dateFinSuspension??null, idAbonnement]
      : [type_id, dateDebut, dateFin, statut, montantDu??null, dureeSuspension??null, causeSuspension??null, dateFinSuspension??null, idAbonnement];
...
ipcMain.handle('getAdherentDetail', async (event, id) => {
  return new Promise((resolve, reject) => {
    autoExpire(() => {
      db.query(
        `SELECT ad.*, ab.idAbonnement, ab.type_id, ab.dateDebut, ab.dateFin,
            ab.statut AS abonnementStatut, ab.dureeSuspension, ab.causeSuspension, ab.dateFinSuspension,
            t.nom AS typeNom, t.prix AS typePrix, t.duree
         FROM Adherent ad
         LEFT JOIN Abonnement ab ON ab.idAbonnement = (SELECT idAbonnement FROM Abonnement WHERE adherent_id = ad.idAdherent ORDER BY dateDebut DESC LIMIT 1)
         LEFT JOIN TypeAbonnement t ON ab.type_id = t.id
         WHERE ad.idAdherent = ?`,
        [id],
        (err, result) => { if (err) reject(err); else resolve(result[0] || null); }
...
ipcMain.handle('updateAdherent', async (event, data) => {
  return new Promise((resolve, reject) => {
    const { idAdherent, nom, prenom, dateNaissance, numTelephone, email, sexe } = data;
    db.query('UPDATE Adherent SET nom=?, prenom=?, dateNaissance=?, numTelephone=?, email=?, sexe=? WHERE idAdherent=?',
      [nom, prenom, dateNaissance, numTelephone, email, sexe, idAdherent],
      (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('searchAdherents', async (event, query) => {
  return new Promise((resolve, reject) => {
    const q = `%${query}%`;
    db.query(
      `SELECT ad.*, ab.statut AS abonnementStatut, ab.dureeSuspension, ab.causeSuspension, ab.dateFinSuspension, t.nom AS typeNom
       FROM Adherent ad
       LEFT JOIN Abonnement ab ON ab.idAbonnement = (SELECT idAbonnement FROM Abonnement WHERE adherent_id = ad.idAdherent ORDER BY dateDebut DESC LIMIT 1)
       LEFT JOIN TypeAbonnement t ON ab.type_id = t.id
       WHERE (ad.nom LIKE ? OR ad.prenom LIKE ? OR ad.email LIKE ? OR ad.numTelephone LIKE ?) AND ab.statut = 'actif'
...
ipcMain.handle('getAdherentsParMois', async () => {
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT DATE_FORMAT(dateCreation, '%Y-%m') AS mois, COUNT(*) AS total FROM Adherent GROUP BY mois ORDER BY mois DESC LIMIT 12`,
      (err, result) => { if (err) reject(err); else resolve(result); }
    );
  });
});
...
ipcMain.handle('getTypeAbonnements', async () => {
  return new Promise((resolve, reject) => {
    db.query('SELECT * FROM TypeAbonnement ORDER BY nom ASC', (err, types) => {
      ...
      db.query('SELECT * FROM Regles', (err2, regles) => {
        if (err2) return reject(err2);
        db.query(`SELECT type_id, COUNT(*) AS nombre_adherents FROM Abonnement WHERE statut = 'actif' GROUP BY type_id`, (err3, counts) => {
          if (err3) return reject(err3);
          resolve(types.map(t => ({
            ...t,
            features: regles.filter(r => r.type_abonnement_id === t.id).map(r => r.description),
            nombre_adherents: Number(counts.find(c => c.type_id === t.id)?.nombre_adherents ?? 0),
          })));
        });
      });
...
ipcMain.handle('deleteTypeAbonnement', async (event, id) => {
  return new Promise((resolve, reject) => {
    db.query('DELETE FROM Abonnement WHERE type_id=?', [id], (err) => {
      if (err) return reject(err);
      db.query('DELETE FROM TypeAbonnement WHERE id=?', [id], (err2, result) => { if (err2) reject(err2); else resolve(result); });
    });
  });
});
...
ipcMain.handle('getAbonnements', async () => {
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT a.*, CONCAT(ad.nom, ' ', ad.prenom) AS adherentNom, t.nom AS typeNom, t.prix AS typePrix
       FROM Abonnement a LEFT JOIN Adherent ad ON a.adherent_id = ad.idAdherent LEFT JOIN TypeAbonnement t ON a.type_id = t.id ORDER BY a.dateDebut DESC`,
      (err, result) => { if (err) reject(err); else resolve(result); }
    );
  });
});
...
ipcMain.handle('getHistoriqueAbonnements', async (event, adherent_id) => {
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT ab.idAbonnement, t.nom AS typeNom, ab.dateDebut, ab.dateFin, ab.statut, ab.montantDu, COALESCE(SUM(p.montant), 0) AS totalPaye
       FROM Abonnement ab LEFT JOIN TypeAbonnement t ON ab.type_id = t.id LEFT JOIN Paiement p ON p.abonnement_id = ab.idAbonnement
       WHERE ab.adherent_id = ? GROUP BY ab.idAbonnement, t.nom, ab.dateDebut, ab.dateFin, ab.statut, ab.montantDu ORDER BY ab.dateDebut DESC`,
      [adherent_id], (err, result) => { if (err) reject(err); else resolve(result); }
    );
  });
});
...
ipcMain.handle('getAbonnementsExpirant', async () => {
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT ad.nom, ad.prenom, ad.numTelephone, ad.email, ab.idAbonnement, ab.dateFin, ab.statut, t.nom AS typeNom, DATEDIFF(ab.dateFin, CURDATE()) AS joursRestants
       FROM Abonnement ab JOIN Adherent ad ON ab.adherent_id = ad.idAdherent JOIN TypeAbonnement t ON ab.type_id = t.id
       WHERE ab.statut = 'actif' AND ab.dateFin BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY) ORDER BY ab.dateFin ASC`,
      (err, result) => { if (err) reject(err); else resolve(result); }
    );
  });
});
...
ipcMain.handle('addAbonnement', async (event, data) => {
  return new Promise((resolve, reject) => {
    const { adherent_id, type_id, dateDebut, dateFin, statut, montantDu, dureeSuspension, causeSuspension, dateFinSuspension } = data;
    db.query(
      `INSERT INTO Abonnement (adherent_id, type_id, dateDebut, dateFin, statut, montantDu, dureeSuspension, causeSuspension, dateFinSuspension) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [adherent_id, type_id, dateDebut, dateFin, statut, montantDu??null, dureeSuspension??null, causeSuspension??null, dateFinSuspension??null],
      (err, result) => { if (err) reject(err); else resolve({ insertId: result.insertId }); }
    );
  });
});
...
ipcMain.handle('getTypesAbonnement', async () => {
  return new Promise((resolve, reject) => {
    db.query('SELECT * FROM TypeAbonnement ORDER BY nom', (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('getPaiements', async () => {
  return new Promise((resolve, reject) => {
    const sql = `
      SELECT p.idPaiement AS id, p.montant, ab.montantDu, p.datePaiement AS datePaiementRaw, p.modePaiement AS mode, CONCAT(a.prenom, ' ', a.nom) AS nom, 'Payé' AS statut
      FROM Paiement p JOIN Abonnement ab ON p.abonnement_id = ab.idAbonnement JOIN Adherent a ON ab.adherent_id = a.idAdherent
      UNION ALL
      SELECT ab.idAbonnement AS id, 0 AS montant, ab.montantDu, ab.dateDebut AS datePaiementRaw, NULL AS mode, CONCAT(a.prenom, ' ', a.nom) AS nom, 'En attente' AS statut
      FROM Abonnement ab JOIN Adherent a ON ab.adherent_id = a.idAdherent
      WHERE ab.montantDu > 0 AND NOT EXISTS (SELECT 1 FROM Paiement p WHERE p.abonnement_id = ab.idAbonnement)
      ORDER BY datePaiementRaw DESC`;
    db.query(sql, (err, results) => { if (err) reject(err); else resolve(results); });
  });
});
...
ipcMain.handle('addPaiement', async (event, data) => {
  return new Promise((resolve, reject) => {
    const { abonnement_id, montant, date, mode } = data;
    const modeMap = { 'carte': 'carte', 'virement': 'virement', 'cash': 'cash', 'Carte bancaire': 'carte', 'Virement': 'virement', 'Espèces': 'cash' };
    db.query('INSERT INTO Paiement (abonnement_id, montant, datePaiement, modePaiement, statut) VALUES (?, ?, ?, ?, ?)',
      [abonnement_id, montant, date, modeMap[mode] || 'cash', 'Payé'],
      (err, result) => { if (err) resolve({ success: false, error: err.message }); else resolve({ success: true, insertId: result.insertId }); });
  });
});
...
ipcMain.handle('getAdherentsWithAbonnement', async () => {
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT ad.*, ab.idAbonnement, ab.type_id, ab.dateDebut, ab.dateFin, ab.statut AS abonnementStatut, ab.dureeSuspension, ab.causeSuspension, ab.dateFinSuspension, t.nom AS typeNom, t.prix AS typePrix
       FROM Adherent ad
       INNER JOIN Abonnement ab ON ab.idAbonnement = (SELECT idAbonnement FROM Abonnement WHERE adherent_id = ad.idAdherent AND statut = 'actif' ORDER BY dateDebut DESC LIMIT 1)
       LEFT JOIN TypeAbonnement t ON ab.type_id = t.id ORDER BY ad.nom ASC`,
      (err, result) => { if (err) reject(err); else resolve(result); }
    );
  });
});
...
ipcMain.handle('getAbonnementsNonPaies', async () => {
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT ad.nom, ad.prenom, ab.idAbonnement, ab.dateDebut, ab.dateFin, ab.statut AS abonnementStatut, t.nom AS typeNom, t.prix AS typePrix, COALESCE(ab.montantDu, t.prix) AS montantDu, COALESCE(SUM(p.montant), 0) AS totalPaye
       FROM Abonnement ab JOIN Adherent ad ON ab.adherent_id = ad.idAdherent JOIN TypeAbonnement t ON ab.type_id = t.id LEFT JOIN Paiement p ON p.abonnement_id = ab.idAbonnement
       WHERE ab.statut = 'actif' GROUP BY ab.idAbonnement, ad.nom, ad.prenom, ab.dateDebut, ab.dateFin, ab.statut, t.nom, t.prix, ab.montantDu
       HAVING COALESCE(ab.montantDu, t.prix) > COALESCE(SUM(p.montant), 0) ORDER BY ad.nom ASC`,
      (err, result) => { if (err) reject(err); else resolve(result); }
    );
  });
});
...
ipcMain.handle('getPaiementsEtAttentes', async () => {
  return new Promise((resolve, reject) => {
    const sqlPaiements = `SELECT CONCAT('P-', p.idPaiement) AS id, p.idPaiement, NULL AS idAbonnement, p.montant, DATE_FORMAT(p.datePaiement, '%d/%m/%Y') AS date, p.modePaiement AS mode, CONCAT(a.prenom, ' ', a.nom) AS nom, a.idAdherent, ab.idAbonnement AS abonnement_id, 'Payé' AS statut, t.nom AS typeNom FROM Paiement p JOIN Abonnement ab ON p.abonnement_id = ab.idAbonnement JOIN Adherent a ON ab.adherent_id = a.idAdherent LEFT JOIN TypeAbonnement t ON ab.type_id = t.id ORDER BY p.datePaiement DESC`;
    const sqlAttentes = `SELECT CONCAT('A-', ab.idAbonnement) AS id, NULL AS idPaiement, ab.idAbonnement, (COALESCE(ab.montantDu, t.prix) - COALESCE(SUM(p.montant), 0)) AS montant, DATE_FORMAT(ab.dateDebut, '%d/%m/%Y') AS date, NULL AS mode, CONCAT(ad.prenom, ' ', ad.nom) AS nom, ad.idAdherent, ab.idAbonnement AS abonnement_id, 'En attente' AS statut, t.nom AS typeNom FROM Abonnement ab JOIN Adherent ad ON ab.adherent_id = ad.idAdherent JOIN TypeAbonnement t ON ab.type_id = t.id LEFT JOIN Paiement p ON p.abonnement_id = ab.idAbonnement GROUP BY ab.idAbonnement, ad.idAdherent, ad.prenom, ad.nom, ab.montantDu, t.prix, ab.dateDebut, t.nom HAVING (COALESCE(ab.montantDu, t.prix) - COALESCE(SUM(p.montant), 0)) > 0 ORDER BY ab.dateDebut DESC`;
    db.query(sqlPaiements, (err1, paiements) => {
      if (err1) return reject(err1);
      db.query(sqlAttentes, (err2, attentes) => {
        if (err2) return reject(err2);
...
ipcMain.handle('getProduits', async () => {
  return new Promise((resolve, reject) => {
    db.query('SELECT * FROM Produit ORDER BY nom ASC', (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('addProduit', async (event, data) => {
  return new Promise((resolve, reject) => {
    const { nom, reference, stock, prix, categorie } = data;
    db.query('INSERT INTO Produit (nom, reference, stock, prix, categorie) VALUES (?, ?, ?, ?, ?)', [nom, reference, stock, prix, categorie],
      (err, result) => { if (err) reject(err); else resolve({ idProduit: result.insertId, status: 'success' }); });
  });
});
...
ipcMain.handle('updateProduit', async (event, data) => {
  return new Promise((resolve, reject) => {
    const { idProduit, nom, reference, stock, prix, categorie } = data;
    db.query('UPDATE Produit SET nom=?, reference=?, stock=?, prix=?, categorie=? WHERE idProduit=?', [nom, reference, stock, prix, categorie, idProduit],
      (err, result) => { if (err) reject(err); else resolve({ status: 'updated', affectedRows: result.affectedRows }); });
  });
});

ipcMain.handle('deleteProduit', async (event, id) => {
  return new Promise((resolve, reject) => {
    db.query('DELETE FROM Produit WHERE idProduit=?', [id], (err, result) => { if (err) reject(err); else resolve({ status: 'deleted' }); });
  });
});

ipcMain.handle('getStatsMagasin', async () => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT (SELECT COUNT(*) FROM Produit) AS totalProduits, (SELECT SUM(stock) FROM Produit) AS totalStock, (SELECT COUNT(*) FROM Produit WHERE stock <= 5) AS alertesStock, (SELECT COUNT(*) FROM HistoriqueVente) AS totalVentes, (SELECT COUNT(*) FROM HistoriqueVente WHERE date = CURDATE()) AS ventesAujourdhui, (SELECT IFNULL(SUM(quantite), 0) FROM HistoriqueVente WHERE date = CURDATE()) AS quantiteVendueAujourdhui, (SELECT COUNT(*) FROM HistoriqueVente WHERE MONTH(date) = MONTH(CURDATE()) AND YEAR(date) = YEAR(CURDATE())) AS ventesCeMois, (SELECT IFNULL(SUM(quantite), 0) FROM HistoriqueVente WHERE MONTH(date) = MONTH(CURDATE()) AND YEAR(date) = YEAR(CURDATE())) AS quantiteVendueCeMois`,
      (err, result) => { if (err) reject(err); else resolve(result[0]); });
  });
});
...
const checkNiveauxExpires = (cb) => {
  db.query(
    `SELECT a.idAdherent, a.niveau, a.niveau_expire, COUNT(hv.id) AS achats_3mois, COALESCE(SUM(hv.quantite * COALESCE(hv.prix_vente, p.prix)), 0) AS depense_3mois
     FROM Adherent a LEFT JOIN HistoriqueVente hv ON hv.adherent_id = a.idAdherent AND hv.date >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH)
     LEFT JOIN Produit p ON p.idProduit = hv.produit_id
     WHERE a.niveau_expire IS NOT NULL AND a.niveau_expire <= CURDATE() AND a.niveau != 'Bronze' GROUP BY a.idAdherent`,
    (err, adherents) => {
      if (err || !adherents.length) { if (cb) cb(); return; }
      const ORDRE = ['Bronze', 'Silver', 'Gold', 'Platine'];
      const updates = adherents.map(ad => {
        const idx = ORDRE.indexOf(ad.niveau);
        const config = NIVEAUX_CONFIG[ad.niveau];
        const aRenouvele = ad.achats_3mois >= config.achatsMois && ad.depense_3mois >= config.depenseMois;
        const nouveauNiveau = aRenouvele ? ad.niveau : ORDRE[Math.max(0, idx - 1)];
        const expire = nouveauNiveau === 'Bronze' ? null : new Date(Date.now() + 90*24*60*60*1000).toISOString().split('T')[0];
        return new Promise(res => { db.query(`UPDATE Adherent SET niveau=?, niveau_depuis=CURDATE(), niveau_expire=? WHERE idAdherent=?`, [nouveauNiveau, expire, ad.idAdherent], () => res()); });
      });
      Promise.all(updates).then(() => { if (cb) cb(); });
    }
  );
...
const updatePointsEtNiveau = (adherentId, cb) => {
  db.query(
    `SELECT FLOOR(COALESCE(SUM(hv.quantite * COALESCE(hv.prix_vente, p.prix)), 0) / 100) AS points FROM HistoriqueVente hv LEFT JOIN Produit p ON p.idProduit = hv.produit_id WHERE hv.adherent_id = ?`,
    [adherentId], (err, rows) => {
      if (err) { if (cb) cb(err); return; }
      const points = rows[0]?.points ?? 0;
      const ORDRE = ['Bronze', 'Silver', 'Gold', 'Platine'];
      let nouveauNiveau = points >= 3000 ? 'Platine' : points >= 1500 ? 'Gold' : points >= 500 ? 'Silver' : 'Bronze';
      db.query('SELECT niveau, niveau_expire FROM Adherent WHERE idAdherent=?', [adherentId], (err2, adRows) => {
        if (err2) { if (cb) cb(err2); return; }
        const niveauActuel = adRows[0]?.niveau ?? 'Bronze';
        const doitMonter = ORDRE.indexOf(nouveauNiveau) > ORDRE.indexOf(niveauActuel);
        const naPasDeTimer = !adRows[0]?.niveau_expire && nouveauNiveau !== 'Bronze';
        if (doitMonter || naPasDeTimer) {
          const expire = nouveauNiveau === 'Bronze' ? null : new Date(Date.now() + 90*24*60*60*1000).toISOString().split('T')[0];
          db.query(`UPDATE Adherent SET points=?, niveau=?, niveau_depuis=CURDATE(), niveau_expire=? WHERE idAdherent=?`, [points, nouveauNiveau, expire, adherentId], (err3) => { if (cb) cb(err3); });
        } else {
          db.query('UPDATE Adherent SET points=? WHERE idAdherent=?', [points, adherentId], (err3) => { if (cb) cb(err3); });
        }
      });
    }
  );
...
ipcMain.handle('getPointsFidelite', async () => {
  return new Promise((resolve, reject) => {
    checkNiveauxExpires(() => {
      db.query(
        `SELECT a.idAdherent, a.nom, a.prenom, a.niveau, a.niveau_depuis, a.niveau_expire, COALESCE(SUM(hv.quantite * COALESCE(hv.prix_vente, p.prix)), 0) AS total_depense, FLOOR(COALESCE(SUM(hv.quantite * COALESCE(hv.prix_vente, p.prix)), 0) / 100) AS points, COUNT(hv.id) AS nb_achats, MAX(hv.date) AS dernier_achat
         FROM Adherent a LEFT JOIN HistoriqueVente hv ON hv.adherent_id = a.idAdherent LEFT JOIN Produit p ON p.idProduit = hv.produit_id
         GROUP BY a.idAdherent, a.nom, a.prenom, a.niveau, a.niveau_depuis, a.niveau_expire ORDER BY points DESC`,
...
ipcMain.handle('updateFideliteApresVente', async (event, adherentId) => {
  return new Promise((resolve, reject) => {
    if (!adherentId) return resolve({ success: true });
    updatePointsEtNiveau(adherentId, (err) => { if (err) reject(err); else resolve({ success: true }); });
  });
});

ipcMain.handle('get-historique-adherent', async (_, adherentId) => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT hv.id, hv.date, hv.quantite, COALESCE(hv.prix_vente, p.prix) AS prix, p.nom AS produit_nom FROM HistoriqueVente hv JOIN Produit p ON p.idProduit = hv.produit_id WHERE hv.adherent_id = ? ORDER BY hv.date DESC`, [adherentId],
      (err, rows) => { if (err) reject(err); else resolve(rows); });
  });
});
...
ipcMain.handle('getUtilisateurs', async () => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT u.*, r.nom AS roleNom FROM Utilisateur u LEFT JOIN Role r ON u.role_id = r.id WHERE u.deleted_at IS NULL`,
      (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('deleteUtilisateur', async (event, id) => {
  return new Promise((resolve, reject) => {
    db.query('UPDATE Utilisateur SET deleted_at = NOW() WHERE idUtilisateur = ?', [id],
      (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('updateUtilisateur', async (event, data) => {
  return new Promise((resolve, reject) => {
    const { idUtilisateur, nom, prenom, email, role_id } = data;
    db.query('UPDATE Utilisateur SET nom=?, prenom=?, email=?, role_id=? WHERE idUtilisateur=?', [nom, prenom, email, role_id, idUtilisateur],
      (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('getRoles', async () => {
  return new Promise((resolve, reject) => {
    db.query('SELECT * FROM Role ORDER BY nom', (err, result) => { if (err) reject(err); else resolve(result); });
  });
});

ipcMain.handle('getCoachs', async () => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT u.idUtilisateur, u.nom, u.prenom FROM Utilisateur u JOIN Role r ON u.role_id = r.id WHERE r.nom = 'coach'`,
      (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('getRolesAvecCount', async () => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT r.id, r.nom AS name, COUNT(u.idUtilisateur) AS users FROM Role r LEFT JOIN Utilisateur u ON u.role_id = r.id GROUP BY r.id, r.nom ORDER BY r.id`,
      (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('getPermissions', async (event, role_id) => {
  return new Promise((resolve, reject) => {
    db.query('SELECT page_key, statut FROM Permissions WHERE role_id = ?', [role_id], (err, result) => {
      if (err) reject(err);
      else { const perms = {}; result.forEach(r => { perms[r.page_key] = r.statut; }); resolve(perms); }
    });
  });
});
...
ipcMain.handle('getStatsPageAdherent', async () => {
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT (SELECT COUNT(*) FROM Adherent) AS total, (SELECT COUNT(DISTINCT ad.idAdherent) FROM Adherent ad JOIN Abonnement ab ON ab.adherent_id = ad.idAdherent WHERE ab.statut = 'actif') AS actifs, (SELECT COUNT(*) FROM Adherent WHERE MONTH(dateCreation) = MONTH(CURDATE()) AND YEAR(dateCreation) = YEAR(CURDATE())) AS nouveauxCeMois`,
      (err, result) => { if (err) reject(err); else resolve(result[0]); }
    );
  });
});
...
ipcMain.handle('savePermissions', async (event, { role_id, permissions }) => {
  return new Promise((resolve, reject) => {
    db.query('DELETE FROM Permissions WHERE role_id = ?', [role_id], (err) => {
      ...
      if (!entries.length) return resolve({ success: true });
      const values = entries.map(([key, statut]) => [role_id, key, statut]);
      db.query('INSERT INTO Permissions (role_id, page_key, statut) VALUES ?', [values], (err2) => { if (err2) reject(err2); else resolve({ success: true }); });
    });
  });
});
...
ipcMain.handle('getStatsAbonnements', async () => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT COUNT(*) AS total, SUM(CASE WHEN statut='actif' THEN 1 ELSE 0 END) AS actifs, SUM(CASE WHEN statut='expiré' THEN 1 ELSE 0 END) AS expires, SUM(CASE WHEN statut='suspendu' THEN 1 ELSE 0 END) AS suspendus FROM Abonnement`,
      (err, result) => { if (err) reject(err); else resolve(result[0]); });
  });
});
...
ipcMain.handle('getAbonnementsParType', async () => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT t.nom AS name, COUNT(*) AS value FROM Abonnement a JOIN TypeAbonnement t ON a.type_id = t.id GROUP BY t.id, t.nom`,
      (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('getAbonnementsExpirantBientot', async () => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT CONCAT(ad.nom, ' ', ad.prenom) AS name, ab.dateFin, DATEDIFF(ab.dateFin, CURDATE()) AS joursRestants FROM Abonnement ab JOIN Adherent ad ON ab.adherent_id = ad.idAdherent WHERE ab.statut = 'actif' AND ab.dateFin >= CURDATE() ORDER BY ab.dateFin ASC LIMIT 5`,
      (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('getFrequentationHebdo', async () => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT DAYNAME(dateDebut) AS day, COUNT(*) AS value FROM Abonnement WHERE dateDebut >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) GROUP BY DAYNAME(dateDebut), DAYOFWEEK(dateDebut) ORDER BY DAYOFWEEK(dateDebut)`,
      (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('getFrequentationSemaine', async () => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT DAYOFWEEK(dateCreation) AS jourNum, COUNT(*) AS total FROM Adherent GROUP BY DAYOFWEEK(dateCreation) ORDER BY DAYOFWEEK(dateCreation)`, (err, result) => {
      if (err) return reject(err);
      const jours = [{jourNum:1,day:"Dim"},{jourNum:2,day:"Lun"},{jourNum:3,day:"Mar"},{jourNum:4,day:"Mer"},{jourNum:5,day:"Jeu"},{jourNum:6,day:"Ven"},{jourNum:7,day:"Sam"}];
      resolve(jours.map(j => ({ day: j.day, value: result.find(r => r.jourNum === j.jourNum)?.total || 0 })));
    });
  });
});
...
ipcMain.handle('addSeanceLibre', async (event, data) => {
  return new Promise((resolve, reject) => {
    const { montant, date, modePaiement, note } = data;
    db.query('INSERT INTO SeanceLibre (montant, date, modePaiement, note) VALUES (?, ?, ?, ?)', [montant, date, modePaiement || 'cash', note || null],
      (err, result) => { if (err) reject(err); else resolve({ success: true, insertId: result.insertId }); });
  });
});

ipcMain.handle('getSeancesLibres', async () => {
  return new Promise((resolve, reject) => {
    db.query('SELECT * FROM SeanceLibre ORDER BY date DESC', (err, result) => { if (err) reject(err); else resolve(result); });
  });
});

ipcMain.handle('getSeancesParJour', async () => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT DAYOFWEEK(date) AS jourNum, COUNT(*) AS total FROM Seance GROUP BY DAYOFWEEK(date) ORDER BY DAYOFWEEK(date)`, (err, result) => {
      if (err) return reject(err);
      const jours = [{jourNum:1,day:"Dim"},{jourNum:2,day:"Lun"},{jourNum:3,day:"Mar"},{jourNum:4,day:"Mer"},{jourNum:5,day:"Jeu"},{jourNum:6,day:"Ven"},{jourNum:7,day:"Sam"}];
      resolve(jours.map(j => ({ day: j.day, value: result.find(r => r.jourNum === j.jourNum)?.total || 0 })));
    });
...
ipcMain.handle('vendreProduit', async (event, { produit_id, utilisateur_id, quantite }) => {
  return new Promise((resolve, reject) => {
    db.query('UPDATE Produit SET stock = stock - ? WHERE idProduit = ? AND stock >= ?', [quantite, produit_id, quantite], (err, result) => {
      if (err) return reject(err);
      if (result.affectedRows === 0) return resolve({ success: false, message: "Stock insuffisant" });
      db.query('INSERT INTO HistoriqueVente (date, utilisateur_id, produit_id, quantite) VALUES (CURDATE(), ?, ?, ?)', [utilisateur_id, produit_id, quantite],
        (err2) => { if (err2) reject(err2); else resolve({ success: true }); });
    });
  });
});
...
ipcMain.handle('createAdherentComplet', async (event, data) => {
  return new Promise((resolve, reject) => {
    const { nom, prenom, dateNaissance, numTelephone, email, sexe, photo, type_id, dateDebut, dateFin, montant, modePaiement, montantDu } = data;
    db.query('INSERT INTO Adherent (nom, prenom, dateNaissance, numTelephone, email, sexe, photo) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [nom, prenom, dateNaissance||null, numTelephone, email||null, sexe, photo||null], (err, resAdherent) => {
        if (err) return reject(err);
        const adherent_id = resAdherent.insertId;
        db.query('INSERT INTO Abonnement (adherent_id, type_id, dateDebut, dateFin, statut, montantDu) VALUES (?, ?, ?, ?, ?, ?)',
          [adherent_id, type_id, dateDebut, dateFin||null, 'actif', montantDu||null], (err2, resAbo) => {
            if (err2) return reject(err2);
            const abonnement_id = resAbo.insertId;
            if (!montant || montant <= 0) return resolve({ success: true, adherent_id, abonnement_id });
            const modeSQL = modePaiement === 'carte' ? 'carte' : modePaiement === 'virement' ? 'virement' : 'cash';
            db.query('INSERT INTO Paiement (abonnement_id, montant, datePaiement, modePaiement, statut) VALUES (?, ?, ?, ?, ?)',
              [abonnement_id, montant, dateDebut, modeSQL, 'Payé'], (err3) => { if (err3) return reject(err3); resolve({ success: true, adherent_id, abonnement_id }); });
          });
      });
  });
});
...
ipcMain.handle('ajouterPaiement', async (event, { abonnement_id, montant, mode, date }) => {
  return new Promise((resolve, reject) => {
    const modeMap = { 'Espèces': 'cash', 'Carte bancaire': 'carte', 'Virement': 'virement' };
    db.query('INSERT INTO Paiement (abonnement_id, montant, datePaiement, modePaiement, statut) VALUES (?, ?, ?, ?, ?)',
      [abonnement_id, montant, date, modeMap[mode] || mode || 'cash', 'Payé'], (err, result) => {
        if (err) return reject(err);
        db.query("UPDATE Abonnement SET statutPaiement = 'payé' WHERE idAbonnement = ?", [abonnement_id], (err2) => {
          if (err2) return reject(err2);
          resolve({ success: true, insertId: result.insertId });
        });
      });
...
ipcMain.handle('addSeance', async (event, data) => {
  return new Promise((resolve, reject) => {
    const { date, heureDebut, heureFin, participantsMax, coach_id, activite_id, publicCible } = data;
    db.query('INSERT INTO Seance (date, heureDebut, heureFin, participantsMax, coach_id, activite_id, publicCible) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [date, heureDebut, heureFin, participantsMax||15, coach_id||null, activite_id||null, publicCible||'Homme'],
      (err, result) => { if (err) reject(err); else resolve({ insertId: result.insertId }); });
  });
});
...
ipcMain.handle('getPresencesSeance', async (event, seance_id) => {
  return new Promise((resolve, reject) => {
    db.query('SELECT adherent_id FROM Presence WHERE seance_id = ?', [seance_id],
      (err, result) => { if (err) reject(err); else resolve(result.map(r => r.adherent_id)); });
  });
});
...
ipcMain.handle('addPresence', async (event, { adherent_id, seance_id, date }) => {
  return new Promise((resolve, reject) => {
    db.query(`INSERT IGNORE INTO Presence (adherent_id, seance_id, date, heureEntree) VALUES (?, ?, CURDATE(), CURTIME())`, [adherent_id, seance_id, date],
      (err, result) => { if (err) return reject(err); resolve({ insertId: result.insertId, alreadyExists: result.affectedRows === 0 }); });
  });
});

ipcMain.handle('getActivites', async () => {
  return new Promise((resolve, reject) => {
    db.query('SELECT * FROM Activite WHERE deleted_at IS NULL ORDER BY nom', (err, result) => { if (err) reject(err); else resolve(result); });
  });
});

ipcMain.handle('addActivite', async (event, data) => {
  return new Promise((resolve, reject) => {
    db.query('INSERT INTO Activite (nom, couleur) VALUES (?, ?)', [data.nom, data.couleur],
      (err, result) => { if (err) reject(err); else resolve({ insertId: result.insertId }); });
  });
});
...
ipcMain.handle('deleteActivite', async (event, id) => {
  return new Promise((resolve, reject) => {
    db.query('UPDATE Activite SET deleted_at = NOW() WHERE idActivite = ?', [id], (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('addTransaction', async (event, data) => {
  ...
  return new Promise((resolve, reject) => {
    if (type === 'achat') {
      db.query('UPDATE Produit SET stock = stock + ? WHERE idProduit = ?', [quantite, produit_id], (err) => {
        if (err) return reject(err);
        db.query('INSERT INTO HistoriqueAchat (date, utilisateur_id, produit_id, quantite, prix_achat) VALUES (NOW(), 1, ?, ?, ?)', [produit_id, quantite, prix||0],
          (err2) => { if (err2) reject(err2); else resolve({ success: true }); });
      });
    } else if (type === 'vente') {
      db.query('SELECT stock, prix FROM Produit WHERE idProduit = ?', [produit_id], (err, rows) => {
        ...
        if (!rows?.length) return reject(new Error('Produit introuvable'));
        if (rows[0].stock < quantite) return reject(new Error('Stock insuffisant'));
        const prixVente = (data.prix_vente != null && data.prix_vente > 0) ? data.prix_vente : rows[0].prix;
        db.query('UPDATE Produit SET stock = stock - ? WHERE idProduit = ?', [quantite, produit_id], (err2) => {
          if (err2) return reject(err2);
          const adhId = adherent_id || null;
          const clientExt = (!adherent_id && client_externe) ? client_externe.trim() : null;
          db.query(`INSERT INTO HistoriqueVente (date, utilisateur_id, produit_id, quantite, prix_vente, adherent_id, client_externe) VALUES (NOW(), 1, ?, ?, ?, ?, ?)`,
            [produit_id, quantite, prixVente, adhId, clientExt], (err3) => {
              if (err3) return reject(err3);
              if (adhId) { updatePointsEtNiveau(adhId, () => resolve({ success: true })); } else { resolve({ success: true }); }
            });
        });
      });
    } else { reject(new Error(`Type inconnu : ${type}`)); }
  });
});

ipcMain.handle('getAdherentNiveau', async (event, adherentId) => {
  return new Promise((resolve, reject) => {
    db.query('SELECT niveau FROM Adherent WHERE idAdherent = ?', [adherentId], (err, rows) => {
      if (err) return reject(err);
      const niveau = rows[0]?.niveau ?? 'Bronze';
      resolve({ niveau, remise: { Bronze: 0, Silver: 5, Gold: 10, Platine: 15 }[niveau] ?? 0 });
    });
  });
});
...
ipcMain.handle('addRole', async (event, { nom }) => {
  return new Promise((resolve, reject) => {
    db.query('INSERT INTO Role (nom) VALUES (?)', [nom.trim()], (err, result) => { if (err) reject(err); else resolve({ insertId: result.insertId }); });
  });
});

ipcMain.handle('deleteRole', async (event, id) => {
  return new Promise((resolve, reject) => {
    db.query('DELETE FROM Role WHERE id=?', [id], (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('deleteSeance', async (event, id) => {
  return new Promise((resolve, reject) => {
    db.query('DELETE FROM Presence WHERE seance_id = ?', [id], (err) => {
      if (err) return reject(err);
      db.query('DELETE FROM Seance WHERE idSeance = ?', [id], (err2, result) => { if (err2) reject(err2); else resolve({ success: true }); });
    });
  });
});
...
ipcMain.handle('getHistoriqueVentes', async () => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT hv.id, hv.date, hv.produit_id, hv.quantite, hv.utilisateur_id, hv.adherent_id, hv.prix_vente, p.nom AS produit_nom, CONCAT(u.prenom, ' ', u.nom) AS utilisateur_nom FROM HistoriqueVente hv LEFT JOIN Produit p ON hv.produit_id = p.idProduit LEFT JOIN Utilisateur u ON hv.utilisateur_id = u.idUtilisateur ORDER BY hv.date DESC`,
      (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('getHistoriqueAchats', async () => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT ha.id, ha.date, ha.produit_id, ha.quantite, ha.prix_achat, ha.utilisateur_id, p.nom AS produit_nom, CONCAT(u.prenom, ' ', u.nom) AS utilisateur_nom FROM HistoriqueAchat ha LEFT JOIN Produit p ON ha.produit_id = p.idProduit LEFT JOIN Utilisateur u ON ha.utilisateur_id = u.idUtilisateur ORDER BY ha.date DESC`,
      (err, result) => { if (err) reject(err); else resolve(result); });
  });
});
...
ipcMain.handle('exportPlanningPDF', async (event, { html, filename }) => {
  const win = new BrowserWindow({ show: false });
  await win.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(html)}`);
  const pdfData = await win.webContents.printToPDF({ printBackground: true, landscape: true, pageSize: 'A4' });
  win.destroy();
  const { filePath } = await dialog.showSaveDialog({ defaultPath: filename || 'planning.pdf', filters: [{ name: 'PDF', extensions: ['pdf'] }] });
  if (filePath) { fs.writeFileSync(filePath, pdfData); return { success: true, filePath }; }
  return { success: false, cancelled: true };
});

ipcMain.handle('sendSpecialMessage', async (event, { type, customText, customSubject }) => {
  const templates = {
    aidkoum: { subject: '🎉 Saha Aidkoum — FitManager', html: `<div style="font-family:sans-serif;padding:24px"><h2>🌙 Saha Aidkoum wa Saha Ftourkoum !</h2><p>Toute l'équipe FitManager vous souhaite une excellente fête de l'Aïd.</p></div>` },
    fermeture: { subject: '⚠️ Fermeture exceptionnelle — FitManager', html: `<div style="font-family:sans-serif;padding:24px"><h2>⚠️ Fermeture exceptionnelle</h2><p>${customText || 'Le club sera fermé exceptionnellement.'}</p></div>` },
    custom: { subject: customSubject || 'Message de FitManager', html: `<div style="font-family:sans-serif;padding:24px"><p>${customText || ''}</p></div>` },
  };
  const tpl = templates[type] || templates.custom;
  const adherents = await query(`SELECT ad.email FROM Adherent ad JOIN Abonnement ab ON ab.adherent_id = ad.idAdherent WHERE ab.statut = 'actif' AND ad.email IS NOT NULL AND ad.email != ''`);
  const emails = adherents.map(a => a.email);
  if (!emails.length) return { success: false, message: 'Aucun email actif trouvé' };
  try {
    await createTransporter().sendMail({ from: `"FitManager" <${process.env.EMAIL_USER}>`, to: emails.join(', '), ...tpl });
    return { success: true, count: emails.length };
...
ipcMain.handle('getSeancesSemaine', async (event, { dateDebut, dateFin }) => {
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT s.*, a.nom AS activiteNom, a.couleur AS activiteCouleur, u.nom AS coachNom, u.prenom AS coachPrenom, COUNT(p.idPresence) AS presents, SUM(CASE WHEN ad.sexe = 'Homme' THEN 1 ELSE 0 END) AS nbHommes, SUM(CASE WHEN ad.sexe = 'Femme' THEN 1 ELSE 0 END) AS nbFemmes, SUM(CASE WHEN ad.sexe = 'Enfant' THEN 1 ELSE 0 END) AS nbEnfants
       FROM Seance s LEFT JOIN Activite a ON s.activite_id = a.idActivite LEFT JOIN Utilisateur u ON s.coach_id = u.idUtilisateur LEFT JOIN Presence p ON p.seance_id = s.idSeance LEFT JOIN Adherent ad ON p.adherent_id = ad.idAdherent
       WHERE s.date BETWEEN ? AND ? GROUP BY s.idSeance ORDER BY s.date ASC, s.heureDebut ASC`,
      [dateDebut, dateFin], (err, result) => { if (err) reject(err); else resolve(result); }
    );
  });
});
...
ipcMain.handle('exportEtEnvoyerPlanningPDF', async (event, { html, filename, dateDebut, dateFin }) => {
  ...
  const pdfData = await win.webContents.printToPDF({ printBackground: true, landscape: true, pageSize: 'A4' });
  win.destroy();
  const adherents = await query(`SELECT ad.email, ad.prenom FROM Adherent ad JOIN Abonnement ab ON ab.adherent_id = ad.idAdherent WHERE ab.statut = 'actif' AND ad.email IS NOT NULL AND ad.email != ''`);
  const emails = adherents.map(a => a.email);
  if (!emails.length) return { success: false, message: 'Aucun email actif trouvé' };
  try {
    await createTransporter().sendMail({
      from: `"FitManager" <${process.env.EMAIL_USER}>`, to: emails.join(', '),
      subject: `📅 Planning mis à jour — ${dateDebut} au ${dateFin}`,
      html: `<div style="font-family:sans-serif;padding:24px"><h2>📅 Le planning a été mis à jour</h2><p>Vous trouverez le nouveau planning en pièce jointe.</p></div>`,
      attachments: [{ filename: filename || 'planning.pdf', content: pdfData, contentType: 'application/pdf' }],
    });
    return { success: true, count: emails.length };
  } catch (err) { return { success: false, error: err.message }; }
});
...
ipcMain.handle('sendRenewalEmail', async (_, data) => {
  ...
  try {
    await createTransporter().sendMail({
      from: '"FitManager" <tinhinanethequeen@gmail.com>', to: email,
      subject: `⚠️ Votre abonnement expire ${expireLabel}`,
      html: `<p>Bonjour <strong>${prenom} ${nom}</strong>,</p><p>Votre abonnement <strong>${type}</strong> expire <strong>${expireLabel}</strong>.</p><p>Merci de votre fidélité 🏋️</p>`,
    });
    return { success: true };
  } catch (err) { return { success: false, error: err.message }; }
});