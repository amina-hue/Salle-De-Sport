...
ipcMain.handle('addPaiement', async (event, data) => {
  return new Promise((resolve, reject) => {
    ...
    db.query(sql, [data.abonnement_id, data.montant, data.date, data.mode], (err, result) => {
      if (err) reject(err);
      else resolve({ success: true });
    });
  });
});
// Récupérer les adhérents AVEC leur abonnement ID pour la modale paiement
  ...
  return new Promise((resolve, reject) => {
    const sql = `
      SELECT 
        ...
        ab.statut
      FROM Adherent ad
      INNER JOIN Abonnement ab ON ad.idAdherent = ab.adherent_id
      WHERE LOWER(ab.statut) = 'actif'
    `;
    db.query(sql, (err, result) => {
      if (err) {
        console.error("ERREUR SQL getAdherentsWithAbonnement:", err);
        reject(err);
      } else {
        console.log("Adhérents trouvés:", result.length); // Pour vérifier dans ta console
        resolve(result);
      }
    });
  });
