...
const StatCard = ({ title, value, sub, icon: Icon, type = "success", trend }) => {

  const themes = {
    success: {
      gradient: "linear-gradient(145deg, #1F2A25 0%, #2F4F3E 100%)",
      border: "rgba(46, 204, 113, 0.2)",
      glow: "rgba(46, 204, 113, 0.25)",
      color: "#2ECC71",
      softBg: "rgba(46, 204, 113, 0.18)",
      softBorder: "rgba(46, 204, 113, 0.3)",
    },
    warning: {
      gradient: "linear-gradient(145deg, #2A1F1A 0%, #5A2E1F 100%)",
      border: "rgba(255, 159, 67, 0.2)",
      glow: "rgba(255, 159, 67, 0.25)",
      color: "#FF9F43",
      softBg: "rgba(255, 159, 67, 0.18)",
      softBorder: "rgba(255, 159, 67, 0.3)",
    },
    danger: {
      gradient: "linear-gradient(145deg, #2A1F1F 0%, #5A1F1F 100%)",
      border: "rgba(255, 77, 77, 0.2)",
      glow: "rgba(255, 77, 77, 0.25)",
      color: "#FF4D4D",
      softBg: "rgba(255, 77, 77, 0.18)",
      softBorder: "rgba(255, 77, 77, 0.3)",
    }
  };
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        ...
        e.currentTarget.style.boxShadow = `0 12px 40px ${theme.glow}`;
      }}
      onMouseLeave={(e) => {
...
,
      ...
      <div style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(33, 31, 31, 0.8)", zIndex: 0 }} />
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "32px 36px", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 32 }}>
          <StatCard  type="success" title="Revenus encaissés" value="10 577 DA" sub="4 paiements" icon={Check} accent="#22c55e" />
          <StatCard type="warning" title="En attente" value="2 887 DA" sub="3 paiements" icon={Clock} accent="#f97316" />
          <StatCard type="danger" title="En retard" value="499 DA" sub="1 paiement" icon={AlertTriangle} accent="#ef4444" />
        </div>
  ...
  marginBottom: 22,
  gap: 14,
  background: "rgba(39, 29, 31, 0.85)",
  padding: "14px 18px",
  borderRadius: 18, // 🔥 plus arrondi
  border: "1px solid rgba(205,73,53,0.25)",
  backdropFilter: "blur(6px)", // effet premium
}}>
  ...
  <div style={{
    ...
    alignItems: "center",
    gap: 10,
    background: "rgba(39, 29, 31, 0.9)",
    border: "1px solid #CD4935",
    borderRadius: 14,
    padding: "12px 16px", // 🔥 plus grand
    flex: 1,
    maxWidth: 460,
  }}>
...
/>
  ...
  <div style={{ display: "flex", gap: 10 }}>
    ...
    <Button variant="filter" icon={Filter} style={{
      padding: "10px 14px",
      fontSize: 13,
      borderRadius: 12
    }}>
     ...
     <Button variant="export" icon={Download} style={{
      padding: "10px 14px",
      fontSize: 13,
      borderRadius: 12
    }}>
      Exporter
        ...
        {/* Table */}
       <div style={{
  background: "linear-gradient(145deg, #271D1F, #1F1618)", // dégradé marron foncé
  borderRadius: 18,
  ...
  <div style={{
    ...
    <span style={{ fontSize: 14, fontWeight: 700, color: "#D9B48F" }}>
      ...
      <span style={{
        ...
        fontSize: 12,
        fontWeight: 600,
        background: "rgba(166,124,82,0.2)", // cuivre clair
        color: "#A67C52", // texte cuivre
        padding: "2px 8px",
        borderRadius: 20
      }}>{filtered.length} résultats</span>
    </span>
  </div>

  <table style={{ width: "100%", borderCollapse: "collapse" }}>
    <thead>
      <tr style={{ background: "#2E1F1B" }}> {/* header marron foncé */}
        {["ID", "ADHÉRENT", "ABONNEMENT", "MONTANT", "DATE", "MÉTHODE", "STATUT", "ACTIONS"].map(h => (
          <th key={h} style={{
            textAlign: "left",
            padding: "11px 22px",
            fontSize: 10,
            color: "#A67C52", // beige/cuivre clair
            letterSpacing: ".9px",
            fontWeight: 700
          }}>{h}</th>
        ))}
    ...
    <tbody>
      {filtered.map((p, i) => {
        ...
        const methode = methodeConfig[p.methode] || { icon: "💰", bg: "rgba(255,255,255,0.1)", color: "#aaa" };
          ...
          <tr key={i} style={{
            ...
            <td style={{ padding: "15px 22px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                }}>{p.nom.split(" ").map(n => n[0]).join("").slice(0, 2)}</div>
            ...
            <td style={{ padding: "15px 22px" }}>
              <span style={{
                ...
                letterSpacing: "-.3px"
              }}>{p.montant.toLocaleString()}<span style={{
                fontSize: 11,
                fontWeight: 600,
                color: "#555",
                marginLeft: 4
            ...
            <td style={{ padding: "15px 22px" }}>
              <span style={{
                ...
                alignItems: "center",
                gap: 5
              }}><span>{methode.icon}</span> {p.methode}</span>
            </td>
            <td style={{ padding: "15px 22px" }}>
              <span style={{
                fontSize: 11,
            ...
            <td style={{ padding: "15px 22px" }}>
              <button style={{
                fontSize: 12,
                color: "#A67C52",
                background: isHovered ? "rgba(166,124,82,0.15)" : "rgba(166,124,82,0.08)",
                ...
                alignItems: "center",
                gap: 5
              }}><Receipt size={12} /> Facture</button>
            </td>
          </tr>
        );
      })}
  ...
  {filtered.length === 0 && <div style={{ padding: "60px 0", textAlign: "center" }}>
    <div style={{ fontSize: 36, marginBottom: 12 }}>🔍</div>
    <div style={{ fontSize: 14, color: "#777" }}>
      Aucun résultat pour "<span style={{ color: "#A67C52" }}>{search}</span>"
    </div>
  </div>}
  ...
  <div style={{
    ...
    <span style={{ fontSize: 12, color: "#A67C52" }}>
      Affichage de <span style={{ color: "#D9B48F", fontWeight: 600 }}>{filtered.length}</span> sur <span style={{ color: "#D9B48F", fontWeight: 600 }}>{paiements.length}</span> transactions
    </span>
    <div style={{ display: "flex", gap: 6 }}>
      {["←", "1", "2", "→"].map((l, i) => (
        <button key={i} style={{
          ...
          fontSize: 12,
          fontWeight: 600,
          background: l === "1" ? "rgba(166,124,82,0.2)" : "#271D1F",
          color: l === "1" ? "#A67C52" : "#777",
          border: l === "1" ? "1px solid rgba(166,124,82,0.3)" : "1px solid rgba(255,255,255,0.08)",
          cursor: "pointer",
          display: "grid",
          placeItems: "center"
        }}>{l}</button>
