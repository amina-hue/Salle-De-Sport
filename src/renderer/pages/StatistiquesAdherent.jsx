...
// const STAT_CARDS = [
//   { label: "Adhérents",          value: "301", trend: "+12% ce mois",     trendColor: "#22c55e", bg: "linear-gradient(135deg,#9F362A,#271D1F)" },
//   { label: "Adhérents actifs",   value: "72",  trend: "+29% ce mois",     trendColor: "#22c55e", bg: "linear-gradient(135deg,#9F362A,#271D1F)" },
//   { label: "Nouveaux adhérents", value: "42",  trend: "+3 cette semaine", trendColor: "#f59e0b", bg: "linear-gradient(135deg,#2D3832,#242227)" },
//   { label: "Total adhérents",    value: "356", trend: "1,4k cours",        trendColor: "#9ca3af", bg: "linear-gradient(135deg,#4B4750,#201F21)" },
// ];

// // ── Style helper pour cacher les scrollbars ──────────────────────────────────
// const noScrollbar = {
//   scrollbarWidth: "none",      // Firefox
//   msOverflowStyle: "none",     // IE / Edge legacy
//   // Chrome / Safari / Edge Chromium → géré via index.css (voir commentaire bas de fichier)
// };
// // Dans ton index.css ou App.css, ajoute une seule fois :
// //   .no-scrollbar::-webkit-scrollbar { display: none; }
// // puis ajoute className="no-scrollbar" aux deux div ci-dessous.
// // ─────────────────────────────────────────────────────────────────────────────
...
//             {[
//               { count: "301", label: "au total",  color: C.muted },
//               { count: "72",  label: "actifs",    color: C.green },
//               { count: "42",  label: "nouveaux",  color: C.gold  },
...
//       {/* ── Content ── CORRECTION 1 : scrollbar cachée sur le scroll principal */}
//       <div
...
//             <h3 style={{ fontFamily: "'Barlow Condensed', sans-serif", color: C.accent, fontWeight: 700, fontSize: "1.1rem", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 16, marginTop: 0 }}>Nouveaux adhérents</h3>

//             {/* CORRECTION 2 : scrollbar cachée sur la liste */}
//             <div
//               className="no-scrollbar"
//               style={{
//                 maxHeight: 320,
...
//                     padding: "10px 14px",
//                     borderBottom: i < newMembersData.length - 1 ? `1px solid ${C.border}` : "none",
//                   }}
//                 >
//                   {/* LEFT SIDE */}
...
//                         fontWeight: 700, color: C.accent, fontSize: "0.75rem", flexShrink: 0,
//                       }}
//                     >
//                       {member.initials}
//                     </div>
...
//                       background: member.status === "En attente" ? "#ca8a04" : "#16a34a",
//                     }}
//                   >
//                     {member.status}
//                   </span>
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // 2. Nouveaux adhérents ce mois (liste)
        const n = await window.api.getAdherentsParMois();
        // getAdherentsParMois retourne [{mois, total}] — on s'en sert pour le graphique
        setGraphData(n.map(r => ({ day: r.mois, value: r.total })));

        // 3. Liste des nouveaux adhérents ce mois pour la card droite
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden", background: C.bg }}>
      ...
      <div className="no-scrollbar" style={{ flex: 1, overflowY: "auto", padding: "24px 36px 40px", ...noScrollbar }}>
        ...
        <div style={{ display: "flex", gap: 20 }}>
          ...
          {/* Line chart — nouveaux adhérents par mois */}
          <div style={{ flex: 1, background: C.card, borderRadius: 14, padding: "20px 22px", border: `1px solid ${C.border}` }}>
