          ...
          <div style={{ width: 380, background: C.card, borderRadius: 14, padding: "20px 22px", border: `1px solid ${C.border}` }}>
            ...
            <div style={{ maxHeight: 320, overflowY: "auto", display: "flex", flexDirection: "column", gap: 0 }}>
              {newMembersData.map((member, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0", borderBottom: i < newMembersData.length - 1 ? `1px solid ${C.border}` : "none" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1 }}>
                    ...
                    <div>
                      <p style={{ color: C.text, fontSize: "0.875rem", fontWeight: 600, margin: 0, fontFamily: "'Barlow', sans-serif" }}>{member.name}</p>
                      <p style={{ color: C.muted, fontSize: "0.72rem", margin: 0 }}>{member.date}</p>
                    </div>
                  </div>
                  <span style={{ padding: "4px 10px", borderRadius: 6, fontSize: "0.72rem", fontWeight: 700, fontFamily: "'Barlow', sans-serif", color: "#fff", background: member.status === "En attente" ? "#ca8a04" : "#16a34a" }}>
                    {member.status}
                  </span>
                </div>
              ))}
