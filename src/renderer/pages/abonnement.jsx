...
const PlanCard = ({ plan }) => {
  ...
  return (
    <div style={{ background: "#1a1a26", borderRadius: 14, padding: "20px", border: `1px solid ${accent}33`, position: "relative", overflow: "hidden", transition: "transform .2s, box-shadow .2s" }}
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
          <div>
            ...
            <p style={{ margin: "4px 0 0", color: "#555", fontSize: 13 }}>Gérez vos types d'abonnements disponibles</p>
          </div>
        ...
        {/* Expiry table */}
        <div style={{ background: "#1a1a26", borderRadius: 16, border: "1px solid #ffffff08", overflow: "hidden" }}>
          <div style={{ padding: "18px 22px 14px", borderBottom: "1px solid #ffffff08" }}>
            <h2 style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>Abonnements arrivant à expiration</h2>
            <p style={{ margin: "3px 0 0", color: "#555", fontSize: 12 }}>Quotas des renouvellements à venir</p>
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#13131e" }}>
                {["ADHÉRENT", "ABONNEMENT", "EXPIRE DANS", "STATUT", "ACTIONS"].map(h => (
                  <th key={h} style={{ textAlign: "left", padding: "11px 22px", fontSize: 10, color: "#555", letterSpacing: ".8px", fontWeight: 700 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {expiryData.map((row, i) => (
                <tr key={i} style={{ borderTop: "1px solid #ffffff06" }}
                  onMouseEnter={e => e.currentTarget.style.background = "#ffffff04"}
                  onMouseLeave={e => e.currentTarget.style.background = ""}
                >
                  <td style={{ padding: "13px 22px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 32, height: 32, borderRadius: "50%", background: avatarColors[i % avatarColors.length] + "33", border: `1px solid ${avatarColors[i % avatarColors.length]}55`, display: "grid", placeItems: "center", fontSize: 11, fontWeight: 700, color: avatarColors[i % avatarColors.length] }}>{row.initials}</div>
                      <span style={{ fontSize: 13, fontWeight: 500 }}>{row.name}</span>
                    </div>
                  </td>
                  <td style={{ padding: "13px 22px", fontSize: 13, color: "#aaa" }}>{row.plan}</td>
                  <td style={{ padding: "13px 22px", fontSize: 13, color: "#ddd", fontWeight: 600 }}>{row.days}</td>
                  <td style={{ padding: "13px 22px" }}>
                    <span style={{ fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: 20, background: row.color + "22", color: row.color }}>
                      {row.status === "urgent" ? "Urgent" : row.status === "warning" ? "Bientôt" : "OK"}
                    </span>
                  </td>
                  <td style={{ padding: "13px 22px" }}>
                    <button style={{ fontSize: 12, color: "#3a7bd5", background: "#3a7bd520", border: "1px solid #3a7bd530", borderRadius: 7, padding: "5px 12px", cursor: "pointer", fontWeight: 600 }}>Contacter</button>
                  </td>
                </tr>
              ))}
            </tbody>
