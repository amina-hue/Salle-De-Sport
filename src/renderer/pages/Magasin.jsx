...
const Magasin = () => {
  ...
  return (
    /* ── pas de app-layout ici ── */
    <div className="magasin-page" style={{ flex: 1, height: "100%", overflowY: "auto" }}>
      <div className="magasin-bg" style={{ backgroundImage: `url(${gymBg})` }} />
        ...
        <div className="magasin-table-section">
          <div className="magasin-table-header">
            ...
            <div className="magasin-table-actions">
              <SearchInput value={search} onChange={handleSearch} placeholder="Rechercher un produit..." />
              <Button variant="primary" icon={Plus}>Ajouter un produit</Button>
            </div>
          </div>

          <table className="magasin-table">
            <thead>
              <tr><th>NOM</th><th>RÉFÉRENCE</th><th>STOCK</th><th>PRIX</th><th>CATÉGORIE</th></tr>
            </thead>
            <tbody>
              {paginated.length > 0 ? paginated.map(p => (
                ...
                <tr><td colSpan={5} className="magasin-empty">Aucun produit trouvé pour « {search} »</td></tr>
              )}
            </tbody>
          </table>
          ...
          {totalPages > 1 && (
            <div className="magasin-pagination">
              <button className="page-nav" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={safePage === 1}>{"< Précédent"}</button>
              ...
              <button className="page-nav" onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={safePage === totalPages}>{"Suivant >"}</button>
            </div>
          )}
        </div>
      </div>
