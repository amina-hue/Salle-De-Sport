...
const Avatar = ({ categorie }) => {
  const config = {
    "Abonnement":   { bg: "#10b98133", border: "#10b98155", color: "#10b981", letter: "A" },
    "Vente":        { bg: "#3b82f633", border: "#3b82f655", color: "#3b82f6", letter: "V" },
    "Séance libre": { bg: "#a78bfa33", border: "#a78bfa55", color: "#a78bfa", letter: "S" },
  };
  const s = config[categorie] || config["Vente"];
  return (
    <div style={{ width: 34, height: 34, borderRadius: "50%", background: s.bg, border: `1.5px solid ${s.border}`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Barlow Condensed', sans-serif", color: s.color, fontSize: "0.75rem", fontWeight: 800, flexShrink: 0 }}>
      {s.letter}
    </div>
  );
};
...
const CategorieBadge = ({ cat }) => {
  const config = {
    "Abonnement":   { bg: "rgba(16,185,129,0.12)",  color: "#10b981", border: "rgba(16,185,129,0.25)"  },
    "Vente":        { bg: "rgba(59,130,246,0.12)",   color: "#60a5fa", border: "rgba(59,130,246,0.25)"  },
    "Séance libre": { bg: "rgba(139,92,246,0.12)",   color: "#a78bfa", border: "rgba(139,92,246,0.25)" },
  };
  const s = config[cat] || config["Vente"];
  return (
    <span style={{ display: "inline-flex", alignItems: "center", borderRadius: 20, padding: "4px 14px", fontSize: "0.72rem", fontWeight: 700, whiteSpace: "nowrap", fontFamily: "'Barlow', sans-serif", background: s.bg, color: s.color, border: `1px solid ${s.border}` }}>
      {cat}
    </span>
  );
};
...
window.api.getSeancesLibres(),      ]);
      ...
      const lignesVentes = ventes.map(v => ({
        id: `v-${v.id}`,
        date: v.date,
        nomComplet: v.produit_nom,
        nom: "Magasin", prenom: "",
        description: v.produit_nom,
        paiement: parseFloat(v.prix_vente) || 0,
        remise: 0,
        categorie: "Vente",
      }));

      const lignesSeances = seancesLibres.map(s => ({
        id: `s-${s.id}`,
        date: s.date,
        nomComplet: s.note || "Séance libre",
        nom: "Séance", prenom: "",
        description: s.note || "Séance libre",
        paiement: parseFloat(s.montant) || 0,
        remise: 0,
        categorie: "Séance libre",
      }));

      setRecettes(
        [...lignesPaiements, ...lignesVentes, ...lignesSeances].sort(
  ...
  const totalSeances      = filtered.filter(r => r.categorie === "Séance libre").reduce((a, r) => a + r.paiement, 0);
  const totalRemises      = filtered.reduce((a, r) => a + r.remise, 0);
  const totalRecettes     = totalAbonnements + totalVentes + totalSeances;
  ...
  if (loading) return (
    <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: C.bg, color: C.muted, fontSize: 14, fontFamily: "'Barlow', sans-serif" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden", position: "relative" }}>
      ...
      <div style={{ position: "fixed", inset: 0, background: "rgba(14,15,17,0.62)", zIndex: -1, pointerEvents: "none" }} />
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 20, marginTop: 12, flexWrap: "wrap" }}>
              {[
                ...
                { count: filtered.filter(r => r.categorie === "Séance libre").length,  label: "séances libres", color: C.purple },
        ...
        <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
              ...
              <div key={label} style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#14161c", border: `1px solid ${C.border}`, borderRadius: 8, padding: "8px 12px" }}>
                  <input type="date" value={value} onChange={e => onChange(e.target.value)}
                    style={{ background: "none", border: "none", color: C.subtle, fontSize: 13, outline: "none", flex: 1, fontFamily: "'Barlow', sans-serif", cursor: "pointer", colorScheme: "dark" }} />
                  <Calendar size={14} color={C.muted} />
                </div>
              </div>
            ))}
            {dateError && (
              <p style={{ color: "#f87171", fontSize: "0.72rem", fontFamily: "'Barlow', sans-serif", fontWeight: 600, margin: 0, lineHeight: 1.4 }}>
                ⚠ {dateError}
              </p>
            )}
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "flex", gap: 16 }}>
              ...
              {/* Séances libres */}
              <div style={{ flex: 1, borderRadius: 14, padding: "20px 22px", background: "linear-gradient(145deg,rgba(139,92,246,0.18),rgba(139,92,246,0.08))", border: "1px solid rgba(139,92,246,0.3)", position: "relative", overflow: "hidden" }}>
                <div style={{ position: "absolute", top: -20, right: -20, width: 80, height: 80, borderRadius: "50%", background: "radial-gradient(circle,rgba(139,92,246,0.25),transparent 70%)" }} />
                <p style={{ color: C.muted, fontSize: "0.72rem", fontFamily: "'Barlow', sans-serif", fontWeight: 600, textTransform: "uppercase", letterSpacing: 1, marginBottom: 6, marginTop: 0 }}>Séances libres</p>
                <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "2rem", fontWeight: 800, color: C.text, margin: "0 0 12px", lineHeight: 1 }}>{totalSeances.toLocaleString("fr-FR")} DA</h2>
                {[
                  { label: "Séances enregistrées", val: filtered.filter(r => r.categorie === "Séance libre").length },
                  { label: "Total encaissé",        val: `${totalSeances.toLocaleString("fr-FR")} DA` },
                ].map((d, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 6 }}>
                    <span style={{ color: C.muted, fontSize: "0.78rem", fontFamily: "'Barlow', sans-serif" }}>{d.label}</span>
                    <span style={{ color: C.subtle, fontSize: "0.78rem", fontWeight: 600, fontFamily: "'Barlow', sans-serif" }}>{d.val}</span>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#14161c" }}>
                {["Date", "Description", "Paiement", "Remise", "Catégorie"].map(h => (
            ...
            <tbody>
              {paginated.length > 0 ? paginated.map(r => (
                <tr key={r.id} style={{ borderBottom: `1px solid ${C.border}`, transition: "background 0.15s" }}
                  ...
                  <td style={{ padding: "14px 22px", color: C.muted, fontSize: "0.78rem", fontFamily: "'Barlow', sans-serif", whiteSpace: "nowrap" }}>
                    {r.date ? FORMAT_DATE(r.date) : "—"}
                  </td>
                  <td style={{ padding: "14px 22px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <Avatar categorie={r.categorie} />
                      <span style={{ color: C.text, fontSize: "0.875rem", fontWeight: 500, fontFamily: "'Barlow', sans-serif" }}>{r.description}</span>
                    </div>
                  </td>
                  <td style={{ padding: "14px 22px" }}>
                    <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "1rem", fontWeight: 800, color: C.text }}>
                      {r.paiement.toLocaleString("fr-FR")}
                      <span style={{ fontSize: "0.72rem", fontWeight: 600, color: C.muted, marginLeft: 4 }}>DA</span>
                    </span>
                  </td>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              {getPages().map((page, idx) => page === "..." ? (
                ...
                <button key={page} onClick={() => setCurrentPage(page)}
                  style={{ width: 32, height: 32, borderRadius: "50%", border: "none", background: page === safePage ? C.accent : "transparent", color: page === safePage ? "#fff" : C.muted, fontSize: "0.82rem", cursor: "pointer", fontWeight: page === safePage ? 700 : 400, fontFamily: "'Barlow', sans-serif", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {page}
                </button>
              ))}
            </div>
            <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={safePage === totalPages}
              style={{ display: "flex", alignItems: "center", gap: 6, background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, color: safePage === totalPages ? C.border : C.muted, fontSize: "0.82rem", fontWeight: 600, padding: "7px 14px", cursor: safePage === totalPages ? "default" : "pointer", fontFamily: "'Barlow', sans-serif", opacity: safePage === totalPages ? 0.4 : 1 }}>
