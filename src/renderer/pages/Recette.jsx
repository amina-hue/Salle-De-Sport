...
const Avatar = ({ initials, role }) => {
  ...
  return <div className="recette-avatar" style={{ background: bg }}>{initials}</div>;
};

const CategorieBadge = ({ cat }) => (
  <span className={`cat-badge ${cat === "Abonnement" ? "cat-abo" : "cat-vente"}`}>{cat}</span>
);
...
const Recette = () => {
  ...
  const handleReset   = () => { setActiveFilter({ debut: "", fin: "" }); setCurrentPage(1); };

  const filtered = useMemo(() => ALL_RECETTES.filter(r => {
  ...
  return (
    /* ── pas de app-layout ici, la sidebar est gérée par le parent ── */
    <div className="recette-page" style={{ flex: 1, height: "100%", overflowY: "auto" }}>
      ...
      <div className="recette-content">
          ...
          <div className="recette-filter-card">
            ...
            <div className="recette-filter-field">
              ...
              <div className="recette-date-wrapper">
                <input type="date" value={dateDebut} onChange={e => setDateDebut(e.target.value)} className="recette-date-input" />
            ...
            <div className="recette-filter-field">
              ...
              <div className="recette-date-wrapper">
                <input type="date" value={dateFin} onChange={e => setDateFin(e.target.value)} className="recette-date-input" />
                <Calendar size={15} color="#888" />
              </div>
            </div>
            <button className="recette-filter-btn" onClick={handleFiltrer}><Filter size={14} /> Filtrer</button>
            {hasActiveFilters && <button className="recette-reset-btn" onClick={handleReset}>Réinitialiser</button>}
          </div>
          ...
          <div className="recette-stats-col">
            <div className="recette-stats-row">
              <div className="recette-stat-card recette-stat-salle">
                ...
                <div className="rsc-detail"><span>Total abonnement</span><span className="rsc-num">{filtered.filter(r => r.categorie === "Abonnement").length}</span></div>
                <div className="rsc-detail"><span className="rsc-remise-label">Remise</span><span className="rsc-remise-val">- {totalRemises.toLocaleString("fr-FR")} DA</span></div>
                <div className="rsc-detail"><span>Paiements encaissés</span><span className="rsc-num">{(totalAbonnements - totalRemises).toLocaleString("fr-FR")} DA</span></div>
              </div>
              <div className="recette-stat-card recette-stat-magasin">
                ...
                <div className="rsc-detail"><span>Produits vendus</span><span className="rsc-num">{filtered.filter(r => r.categorie === "Vente").length}</span></div>
                <div className="rsc-detail"><span>Tickets encaissés</span><span className="rsc-num">{totalVentes.toLocaleString("fr-FR")} DA</span></div>
              </div>
            </div>
        ...
        <div className="recette-table-section">
          ...
          <table className="recette-table">
            <thead>
              <tr><th>DATE</th><th>NOM COMPLET</th><th>DESCRIPTION</th><th>PAIEMENT</th><th>REMISE</th><th>CATÉGORIE</th></tr>
            </thead>
            <tbody>
              {paginated.length > 0 ? paginated.map(r => (
                <tr key={r.id}>
                  <td className="rtd-date">{FORMAT_DATE(r.date)}</td>
                  <td><div className="recette-nom-cell"><Avatar initials={r.initials} role={r.role} /><span className="recette-nom-text">{r.nom}</span></div></td>
                  <td className="rtd-desc">{r.description}</td>
                  <td className="rtd-paiement">{r.paiement.toLocaleString("fr-FR")} DA</td>
                  <td className="rtd-remise">{r.remise > 0 ? `${r.remise.toLocaleString("fr-FR")} DA` : "0 DA"}</td>
                  <td><CategorieBadge cat={r.categorie} /></td>
                </tr>
              )) : (
                <tr><td colSpan={6} className="recette-empty">Aucune recette pour cette période.</td></tr>
              )}
            </tbody>
          </table>

          <div className="recette-pagination">
            <button className="page-nav" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={safePage === 1}><ChevronLeft size={14} /> Page précédente</button>
            <div className="page-numbers">
              ...
              {getPages().map((page, idx) => page === "..." ? (
                ...
                <button key={page} className={`page-btn ${page === safePage ? "active" : ""}`} onClick={() => setCurrentPage(page)}>{page}</button>
              ))}
            </div>
            <button className="page-nav" onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={safePage === totalPages}>Page suivante <ChevronRight size={14} /></button>
          </div>
        </div>
