...
export default function QuickActions({ typesAbonnement, handleSaveAdherent, handleSaveAbonnement, handleSavePaiement }) {
  ...
  const [activeModal, setActiveModal] = useState(null); // ✅ was missing

  const user = JSON.parse(localStorage.getItem("user"));
  const navigate = useNavigate();

  const open = (modal) => {
    setActiveModal(modal); // ✅ was missing (was calling window.open())
    setShowActions(false);
  };

  const close = () => setActiveModal(null); // ✅ was missing
