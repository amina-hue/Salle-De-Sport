...
import React, { useState, useEffect } from 'react';
import GYM_BG from '../../images/gym.png';
import { Search, ChevronDown, ChevronUp, User, CheckCircle2, Clock } from 'lucide-react';

const C = {
  accent: '#e53935', text: '#f0f0f0', muted: '#7a7f8e',
  bgCard: '#1a1516', bgInput: 'rgba(255,255,255,0.06)',
  border: '#333', green: '#22c55e', gold: '#f59e0b'
};
...
export default function NouveauPaiementModal({ onSave, onClose }) {
  ...
  const handleConfirm = () => {
    ...
    // NOUVEAU : Envoie le statut au backend
    onSave?.({
      abonnement_id: selectedAbo.idAbonnement,
      montant:       resteAPayer,
      mode,
      date,
      statut: statut  // ← AJOUTÉ : "Payé" ou "En attente"
    });
  };
        ...
        <div style={{ padding: '20px 25px 25px', display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          {selectedAbo && (
            <div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                {[
                  { value: 'Payé', label: '✅ Payé maintenant', icon: <CheckCircle2 size={16} />, color: C.green, desc: 'Encaissé immédiatement' },
                  { value: 'En attente', label: '⏳ Payer plus tard', icon: <Clock size={16} />, color: C.gold, desc: 'Apparaitra en attente' }
                ].map((option) => {
                  const selected = statut === option.value;
                  return (
                    <button
                      key={option.value}
                      type="button"
                      ...
                      style={{
                        ...
                        border: selected ? `2px solid ${option.color}` : '1px solid #444',
                        background: selected ? `${option.color}15` : 'rgba(255,255,255,0.03)',
                        color: selected ? option.color : C.text,
                        display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
                        fontWeight: selected ? 700 : 500, fontSize: '0.8rem', transition: 'all 0.2s'
                      }}
                    >
                      {option.icon}
                      <div style={{ fontSize: '0.7rem', opacity: 0.9 }}>{option.desc}</div>
                    </button>
                  );
                })}
              </div>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 4 }}>
            ...
            >
              CONFIRMER {statut === 'En attente' ? '(En attente)' : '(Payé)'}
            </button>
          </div>
